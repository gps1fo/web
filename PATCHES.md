...
| P-003 | Le service worker de la carte (PWA) renvoyait SA page pour toute navigation de l'origine hors `/api/` : `/console/` affichait une carte vide, lien de menu compris, chez tout utilisateur connecté une fois à la carte | `vite.config.js` (1 ligne : `/^\/(console|auth|espace)\//` ajouté à `navigateFallbackDenylist`, l'élément amont inchangé) | Non : c'est la configuration de construction du worker, aucun point d'accroche ailleurs | Non : les chemins sont les nôtres | 2026-09-25 |
